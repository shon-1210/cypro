      ...
      oe <- shiny::observeEvent(input$add_well_plate, {
        ...
        if(editing_well_plate()){
          ...
          exp_design <- 
            setWellPlate(
              object = exp_design, 
              well_plate_object = well_plate_obj
            )
      ...
      output$well_plate_folders <- DT::renderDataTable({
        ...
        df <- 
          data.frame(
            ...
            "Type" = purrr::map_chr(.x = well_plate_list(), ~ getWellPlateType(object = .x)), 
            "ROIS per Well" = purrr::map_dbl(.x = well_plate_list(), ~ base::unique(.x@layout[["n_rois"]]))
          )
